document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-slider]').forEach(slider => {
    const imgs = [...slider.querySelectorAll('.project-media img')];
    const thumbs = [...slider.querySelectorAll('.project-thumb')];
    let index = 0;
    const show = i => {
      if (!imgs.length) return;
      index = (i + imgs.length) % imgs.length;
      imgs.forEach((img, n) => img.classList.toggle('is-active', n === index));
      thumbs.forEach((thumb, n) => thumb.classList.toggle('is-active', n === index));
    };
    thumbs.forEach((thumb, n) => thumb.addEventListener('click', e => {
      e.preventDefault();
      show(n);
    }));
  ...
  document.querySelectorAll('input[type="file"]').forEach(input => {
    ...
    input.addEventListener('change', () => {
      ...
      if (label) label.textContent = files.length ? files.join(', ') : 'Можно прикрепить фото крыши, замеры или готовую смету';
  ...
  document.querySelectorAll('[data-review-slider]').forEach(slider => {
    ...
    let index = 0;
    const visible = () => window.matchMedia('(max-width:720px)').matches ? 1 : window.matchMedia('(max-width:1024px)').matches ? 2 : 4;
  ...
  document.querySelectorAll('[data-form]').forEach(form => {
    form.addEventListener('submit', e => {
      ...
      if (status) status.textContent = 'Спасибо, заявка принята. Мы скоро свяжемся.';
      form.querySelectorAll('button').forEach(button => button.disabled = true);
      setTimeout(() => form.querySelectorAll('button').forEach(button => button.disabled = false), 1600);
    });
  });
  ...
  document.querySelectorAll('[data-quiz]').forEach(quiz => {
    ...
    const label = quiz.querySelector('[data-quiz-step-label]');
  ...
  document.querySelectorAll('[href="#lead"],[data-open-modal]').forEach(btn => btn.addEventListener('click', e => {
    e.preventDefault();
    document.getElementById('lead-modal')?.classList.add('is-open');
  }));
  document.querySelectorAll('[href="#messengers"],[data-open-messengers]').forEach(btn => btn.addEventListener('click', e => {
    e.preventDefault();
    document.getElementById('messenger-modal')?.classList.add('is-open');
  }));
  ...
  if (counters.length) {
    ...
    counters.forEach(el => io ? io.observe(el) : runCounter(el));
  }

  document.querySelectorAll('.modal__close,.modal').forEach(el => el.addEventListener('click', e => {
    if (e.target === el || el.classList.contains('modal__close')) {
